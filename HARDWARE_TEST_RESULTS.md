...
- Auto-capture can now omit `--disc-type`; it uses the player-reported CAV/CLV disc type after spin-up and still rejects unknown or mismatched explicit types.
- CAV partial auto-capture now logs when it sees still-frame and successfully resumes playback, making stop-code handling visible during hardware tests.
- `cmake --build build` and `ctest --test-dir build --output-on-failure` passed after these CAV changes.
