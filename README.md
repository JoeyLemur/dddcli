...
Before the first non-root capture, complete the [Linux capture host setup](docs/LINUX_CAPTURE_HOST_SETUP.md). It puts USB-device permissions, the USBFS transfer-memory pool, and the `memlock`/`rtprio` process limits in one place. The default queue expects `usbfs_memory_mb=512`, `memlock=524288` KiB (or unlimited), and `rtprio=80` or higher.
